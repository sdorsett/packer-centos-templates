...
echo "###### build the vuaas packer template - centos-68-puppet-432-$VERSION-$PADDEDBUILD.ova ######"
packer build -var-file=/root/packer-remote-info.json templates/centos-6.8-puppet-4.3.2.json

echo "###### add vapp properties to .ovf file ######"
sed -i -e '/  <\/VirtualSystem>/d' centos-68-puppet-432/centos-68-puppet-432/centos-68-puppet-432.ovf
sed -i -e '/<\/Envelope>/d' centos-68-puppet-432/centos-68-puppet-432/centos-68-puppet-432.ovf 
sed -i -e 's/<VirtualHardwareSection>/<VirtualHardwareSection ovf:transport="com.vmware.guestInfo">/g' centos-68-puppet-432/centos-68-puppet-432/centos-68-puppet-432.ovf 
...
echo "###### delete centos-68-puppet-432-$VERSION-$PADDEDBUILD.ova if it exists  ######"
rm -rf /root/packer-build-artifacts/centos-68-puppet-432-$VERSION-$PADDEDBUILD.ova

echo "###### convert the multiple exported .ovf files to a single .ova file ######"
ovftool --skipManifestCheck centos-68-puppet-432/centos-68-puppet-432/centos-68-puppet-432.ovf /root/packer-build-artifacts/centos-68-puppet-432-$VERSION-$PADDEDBUILD.ova
